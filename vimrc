...
augroup multiWindowOptions
  ...
  nnoremap <expr> H (filereadable(expand('<cfile>')))?'<C-w>vgf':'H'
  nnoremap <expr> L (filereadable(expand('<cfile>')))?'<C-w>v<C-w>lgf':'L'
  nnoremap <expr> K (filereadable(expand('<cfile>')))?'<C-w>f':'K'
  nnoremap <expr> J (filereadable(expand('<cfile>')))?'<C-w>s<C-w>jgf':'J'
augroup END

augroup termMaps
  autocmd!
  vnoremap <expr> p (&buftype ==# 'terminal')?'I<C-w>""':'p'
  nnoremap <expr> p (&buftype ==# 'terminal')?'I<C-w>""':'p'
augroup END
